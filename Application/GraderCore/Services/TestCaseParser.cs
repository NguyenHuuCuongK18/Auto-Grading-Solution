...
namespace GraderCore.Services
{
    ...
    public class TestCaseParser : ITestCaseParser
    {
        ...
        /// <summary>
        /// Loads test case configuration from header.xlsx
        /// </summary>
        ...
        private Dictionary<int, UserAction> LoadUserActions(XLWorkbook workbook)
        {
            ...
            foreach (var row in rows)
            {
                var stageCell = row.Cell(1);              // Stage
                var input = row.Cell(2).GetString();      // Input
                var action = row.Cell(3).GetString();     // Action
        ...
        private void LoadClientExpectations(XLWorkbook workbook, List<TestStage> stages)
        {
            ...
            {
                var stageCell = row.Cell(1);              // Stage
                var console = row.Cell(2).GetString();    // Console
        ...
        private void LoadNetworkExpectations(XLWorkbook workbook, List<TestStage> stages)
        {
            ...
            {
                var stageCell = row.Cell(1);                 // Stage
                var url = row.Cell(2).GetString();           // Url
                var reqPayload = row.Cell(3).GetString();    // REQ_Payload
                var resPayload = row.Cell(4).GetString();    // RES_Payload
